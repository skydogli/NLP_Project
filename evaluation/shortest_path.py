...
assert args.prompt in ["CoT", "none", "0-CoT", "LTM", "PROGRAM","k-shot","Algorithm","Instruct",'dot1','dot2','ins1','ins2','ins3','skydogli']
...
def translate(G, q, args):
    ...
    if args.prompt in ["CoT", "k-shot","Algorithm","Instruct",'dot1','dot2','ins1','ins2','ins3','skydogli']:
        with open("NLGraph/shortest_path/"+prompt_folder+"/" + args.prompt + "-prompt.txt", "r") as f:
            exemplar = f.read()
        Q = Q + exemplar + "\n\n\n"
    if args.city == 0:
        Q = Q + "In an undirected graph, the nodes are numbered from 0 to " + str(n-1)+", and the edges are:\n"
    else:
        Q = Q + "In a country, the cities are numbered from 0 to " + str(n-1)+". There are roads between the cities, and the roads are:\n"
    if args.city == 0:
        for i in range(len(edge)):
            Q = Q + 'an edge between node '+str(edge[i][0])+' and node '+str(edge[i][1]) + " with weight " + str(G[edge[i][0]][edge[i][1]]["weight"])
            if i + 1 == len(edge):
                Q = Q + '.'
            else:
                Q = Q + ','
        ...
        for i in range(len(edge)):
            Q = Q + 'a road between city '+str(edge[i][0])+' and city '+str(edge[i][1]) + " with length " + str(G[edge[i][0]][edge[i][1]]["weight"])
            if i + 1 == len(edge):
                Q = Q + '.'
            else:
                Q = Q + ','
    ...
    if args.city == 0:
        ...
        Q = Q + "Q: Give the shortest path from city " + str(q[0])+" to city " + str(q[1]) + ".\nA:"
...
def log(Q, res1, res2, answer, args):
    ...
    newpath = 'log/shortest_path/'+args.model+'-'+args.mode+ '-'+time+ '-' + args.prompt
    if args.city == 1:
...
def main():
    ...
    for i in tqdm(range((g_num + batch_num - 1) // batch_num)):
        ...
        for j in range(i*batch_num, min(g_num, (i+1)*batch_num)):
            with open("NLgraph/shortest_path/graph/"+args.mode+"/standard/graph"+str(j)+".txt","r") as f:
