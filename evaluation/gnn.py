...
parser.add_argument('--layer', type=int, default=1, help='number of GNN layers (default: 1)')
...
assert args.prompt in ["CoT", "none", "0-CoT", "LTM", "PROGRAM","k-shot","skydogli"]
...
def translate(G, embedding, args):
    ...
    if args.prompt in ["CoT", "k-shot"]:
        if args.layer == 2:
            ...
            with open("NLGraph/GNN/one-prompt/" + args.prompt + "-prompt.txt", "r") as f:
                exemplar = f.read()
        Q = Q + exemplar + "\n\n\n"
    Q = Q + "In an undirected graph, the nodes are numbered from 0 to " + str(n-1)+", and every node has an embedding. (i,j) means that node i and node j are connected with an undirected edge.\n"
    Q = Q + "Embeddings:\n"
    for i in range(n):
        Q = Q + "node "+str(i)+": ["
    ...
    Q = Q + '\n'
    Q = Q + 'In a simple graph convolution layer, each node\'s embedding is updated by the sum of its neighbors\' embeddings.\n'
    word = "two layers"
    if args.layer == 1:
        word = "one layer"
    Q = Q + "Q: What's the embedding of each node after "+word+" of simple graph convolution layer?" + "\nA:"
    match args.prompt:
        case "0-CoT":
...
def main():
    ...
    for i in tqdm(range((g_num + batch_num - 1) // batch_num)):
        ...
        for j in range(i*batch_num, min(g_num, (i+1)*batch_num)):
            with open("NLgraph/GNN/graph/"+args.mode+"/standard/graph"+str(j)+".txt","r") as f:
