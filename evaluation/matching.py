...
def translate(G, n1, n2, args):
    ...
    for i in range(len(edge)):
        Q = Q + 'Applicant '+str(edge[i][0])+' is interested in job '+str(edge[i][1]-n1)+'.\n'
    Q = Q + "Q: Find an assignment of jobs to applicants in such that the maximum number of applicants find the job they are interested in."
    # if args.prompt != "none":
...
def main():
    ...
    for i in tqdm(range((g_num + batch_num - 1) // batch_num)):
        ...
        for j in range(len(Q_list)):
            vote1 = 0
            for k in range(sc):
                ans, G, std, n1 = sc_list[k][j].lower(), G_list[j], std_list[j], n1_list[j]
                answer.append(ans.lower())
                try:
                    r1 = evaluate(ans.lower(), G, n1, std) # r1 for solution check and r2 for final answer check
                    vote1 += r1
                except:
                    print(ans.lower())
